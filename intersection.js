...
getLightPosition(direction) {
    ...
    getProperExitPoint(fromDirection, toDirection, turnType) {
        ...
        if (turnType === CONFIG.TURN_TYPES.RIGHT) {
            switch (fromDirection) {
                case CONFIG.DIRECTIONS.NORTH:
                case CONFIG.DIRECTIONS.SOUTH:
                    // Right turn from N/S goes into same lane as W→E straight
                    return { x: CONFIG.CANVAS_WIDTH, y: this.centerY - laneOffset };
                case CONFIG.DIRECTIONS.WEST:
                    // Right turn from W→E goes into same lane as N→S straight
                    return { x: this.centerX - laneOffset, y: CONFIG.CANVAS_HEIGHT };
                case CONFIG.DIRECTIONS.EAST:
                    // Right turn from E→W goes into same lane as S→N straight
                    return { x: this.centerX + laneOffset, y: 0 };
            }
        } else if (turnType === CONFIG.TURN_TYPES.LEFT) {
            switch (fromDirection) {
                case CONFIG.DIRECTIONS.NORTH:
                case CONFIG.DIRECTIONS.SOUTH:
                    // Left turn from N/S goes into same lane as E→W straight
                    return { x: 0, y: this.centerY + laneOffset };
                case CONFIG.DIRECTIONS.WEST:
                    // Left turn from W→E goes into same lane as S→N straight
                    return { x: this.centerX + laneOffset, y: 0 };
                case CONFIG.DIRECTIONS.EAST:
                    // Left turn from E→W goes into same lane as N→S straight
                    return { x: this.centerX - laneOffset, y: CONFIG.CANVAS_HEIGHT };
            }
        } else {
...
// Example usage
const intersection = new Intersection(400, 300);

// Assuming 'direction' is defined based on your application logic
if (typeof intersection.getExitpoint === 'function' && direction) {
    const exitPoint = intersection.getExitpoint(direction);
    // ... use exitPoint ...
} else {
    console.warn("getExitpoint is not a function or direction is undefined");
}